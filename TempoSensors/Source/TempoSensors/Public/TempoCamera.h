...
{
	TempoSensors::DepthImageRequest Request;
	TResponseDelegate<TempoSensors::DepthImage> ResponseContinuation;
};

struct FBoundingBoxesRequest
{
	TempoSensors::BoundingBoxesRequest Request;
...
class TEMPOSENSORS_API UTempoCamera : public UTempoTiledSceneCaptureComponent
{
	GENERATED_BODY()
...
public:
	...
	void RequestMeasurement(const TempoSensors::ColorImageRequest& Request, const TResponseDelegate<TempoSensors::ColorImage>& ResponseContinuation);
...
protected:
	...
	// requests are pending. TPimplPtr (not TUniquePtr) lets the wrapper stay forward-declared in a
	// UObject: its type-erased deleter is safe in UHT's generated vtable-helper constructor.
