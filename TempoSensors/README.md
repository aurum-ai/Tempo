...
## Migrating clients (API v0.1.1 → v0.2.0)

v0.2.0 changes the per-return / per-pixel sensor payloads from `repeated float` (and `repeated uint32`) to opaque `bytes`. The motivation is throughput: a packed numeric field forces the client to materialize every element on decode, while a `bytes` blob can be reinterpreted in place (`np.frombuffer`, `f32::from_le_bytes`, `reinterpret_cast`) with zero per-element cost. On large payloads — a 2000×128 lidar or a 1080p depth frame — this is the difference between the client keeping up and falling behind.
...
Field numbers are unchanged. The layout (H-outer / V-inner for lidar, row-major for depth) is unchanged. Only the representation changed.

### What breaks, and when

There are two distinct compatibility surfaces — get them straight before you upgrade:

- **Wire format.** For the five **float** fields, packed `repeated float` is byte-identical to a `bytes` blob of little-endian float32 at the same field number, so they stay wire-compatible in both directions: an old client (old stubs) talking to a new server still decodes them correctly. **`labels` is the exception** — packed `repeated uint32` is varint-encoded, while the new `bytes` is fixed 4-byte little-endian. An old client reading `labels` from a new server will **silently decode garbage** (same wire type, so no error is raised). Treat `labels` as a hard, version-coupled break.
- **Source / API.** The moment a client **regenerates its stubs** from the v0.2.0 protos, every field above becomes a `bytes` object. Any code that treated them as a numeric sequence must change — and not all of the breakage is loud:
  - `np.asarray(field, dtype=np.float32)` → raises `ValueError: could not convert string to float` (loud).
  - `len(field)` → returns the **byte** count (4× the element count), not the number of returns (silent).
  - `list(field)` / `for v in field` / `field[i]` → yields raw byte integers, not floats (silent).

The silent cases are the dangerous ones. Audit every read of these six fields, not just the ones that crash.

### Migrating Python clients

Replace `np.asarray` (and any iteration / `len`) on these fields with `np.frombuffer`:
...
If you use the shipped helpers (`tempo_sim.TempoImageUtils`, `tempo_sim.TempoLidarUtils`) instead of decoding the fields yourself, there's nothing to do — they already use `np.frombuffer`. Note that the hand-written helper wrappers are **not** regenerated by proto codegen, so a partial upgrade (new `*_pb2.py`, stale wrappers) will produce the `np.asarray` error above; reinstall the whole `TempoCore/Content/Python/API` package together.

`np.frombuffer` returns a read-only view over the message buffer. Call `.copy()` if you need to mutate it or hold it past the lifetime of the message.

### Migrating Rust / C++ clients

The same representation change applies. The generated accessors return the raw buffer (`Vec<u8>` / `&[u8]` in Rust, `std::string` in C++) instead of a repeated-field container:

- **Rust:** decode with `bytes.chunks_exact(4).map(|c| f32::from_le_bytes(c.try_into().unwrap()))` (or `u32::from_le_bytes` for `labels`); see `ExampleClients/Rust/SensorPlayground`.
- **C++:** the element count is now `field().size() / sizeof(float)` (or `/ sizeof(uint32_t)` for `labels`), and the data pointer is `reinterpret_cast<const float*>(field().data())`; `field_size()` no longer exists. See `ExampleClients/cpp/SensorPlayground`.
